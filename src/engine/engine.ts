...
export class UniverseEngine {
  ...
  private buildMultiverse(customRealitiesList?: RealityConfig[]) {
    ...
    const marbleGlassVert = `varying vec3 vN; varying vec3 vV; void main(){ vN = normalize(normalMatrix * normal); vec4 mv = modelViewMatrix * vec4(position,1.0); vV = normalize(-mv.xyz); gl_Position = projectionMatrix * mv; }`;
    const marbleGlassFrag = `uniform vec3 uColorA; uniform vec3 uColorB; uniform float uTime; varying vec3 vN; varying vec3 vV;
void main(){
  float f = pow(1.0 - abs(dot(normalize(vN), normalize(vV))), 1.9);
  float band = 0.5 + 0.5 * sin(vN.y * 5.0 - uTime * 0.5);
  vec3 rim = mix(uColorA, uColorB, 0.35 + 0.3 * band);
  gl_FragColor = vec4(rim * f * 2.2 + rim * 0.05, min(1.0, f * 1.3 + 0.04));
}`;
  ...
  zoomToHierarchy(stageIndex: number) {
    ...
    switch (stageIndex) {
      case 0: // Multiverse
        this.rig.setZoomTarget(0.96); this.rig.setOrbit(null, 1.05); break;
      case 1: // Reality / Universe
        this.rig.setZoomTarget(0.88); this.rig.setOrbit(null, 1.05); break;
      case 2: // Cosmic Web
        this.rig.setZoomTarget(0.82); this.rig.setOrbit(null, 1.05); break;
      case 3: // Supercluster Complex
        this.rig.setZoomTarget(0.76); this.rig.setOrbit(null, 1.05); break;
      case 4: // Supercluster
        this.rig.setZoomTarget(0.71); this.rig.setOrbit(null, 1.05); break;
      case 5: // Galaxy Cluster / Group
        this.rig.setZoomTarget(0.66); this.rig.setOrbit(null, 1.05); break;
      case 6: // Galaxy
        this.rig.setZoomTarget(0.60); this.rig.setOrbit(null, 1.08); break;
      case 7: // Galactic Region
        this.rig.setZoomTarget(0.52); this.rig.setOrbit(null, 1.10); break;
      case 8: // Spiral Arm
        this.rig.setZoomTarget(0.44); this.rig.setOrbit(null, 1.10); break;
      case 9: // Star-Forming Region
        this.rig.setZoomTarget(0.33); this.rig.setOrbit(null, 1.12); break;
      case 10: // Stellar System
      default:
        this.rig.setZoomTarget(0.15); this.rig.setOrbit(null, 1.12); break;
    }
  }
  zoomIn(step = 0.12) {
  ...
  private tick = () => {
    ...
    if (
      !this.focusId && !this.coreActive && this.rig.dist() < 150 && this.portal.phase === 'idle'
    ...
    if (activeFb) activeFb.group.getWorldPosition(this._vFocusScratch);
    else this._vFocusScratch.set(0, 0, 0);
    this.rig.update(dt, {
      focus: this._vFocusScratch,
      focused: !!activeFb,
      focusRadius: activeFb ? activeFb.data.radius : 6,
      portalEase: ease,
    });
