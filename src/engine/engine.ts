...
export class UniverseEngine {
  ...
  constructor(canvas: HTMLCanvasElement, bodies: CosmicBody[], cb: EngineCallbacks) {
    ...
    window.addEventListener('eventide-vault-pulse', ((e: CustomEvent<{ intensity?: number }>) => {
      this.vaultPulse = Math.min(1.5, this.vaultPulse + (e.detail?.intensity ?? 1));
    }) as EventListener);
...
void main(){
  ...
  private tick = () => {
    ...
    /* auto engage — ONLY while actively zooming in: diving toward a world
       ...
       random body — and the boot fall is scripted, so it never engages there. */
    if (
