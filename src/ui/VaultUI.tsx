...
const RUNNER_LABEL: Record<RunnerKind, string> = {
  'web-app': 'web runtime · sandboxed iframe',
  javascript: 'js runtime · isolated worker',
  python: 'python runtime · pyodide wasm',
  pdf: 'document runtime · native viewer',
...
function WebAppRun({ file, blob }: { file: VaultFile; blob: Blob }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        /* siblings from the same vault folder become live blob assets */
        const siblings = new Map<string, Blob>();
        const folder = file.folder || '/';
        for (const f of state.vault) {
          if (f.id === file.id || (f.folder || '/') !== folder) continue;
  ...
  return (
    <div>
      <div className="flex items-center gap-3 px-3 h-9 border border-line/60 bg-void/50">
        ...
        <span className="font-mono text-[8.5px] text-slate-dim">no network · no vault access · scripts sandboxed</span>
        <div className="flex-1" />
      ...
      <iframe key={nonce} src={url} title={file.name} sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-popups"
        className="w-full h-[58vh] mt-2 bg-white border border-line/60" />
    </div>
  );
}
...
function JsRun({ blob }: { blob: Blob }) {
  ...
  const start = async () => {
    setLines([{ t: 'booting isolated worker…', level: 'sys' }]);
    try {
    ...
    <div>
      {!running && (
        <div className="text-center py-8">
          <p className="font-mono text-[10px] tracking-[0.22em] uppercase text-slate-dim">javascript payload · runs in an isolated worker (no dom, no vault access)</p>
...
function PyRun({ blob }: { blob: Blob }) {
  ...
  const start = async () => {
    ...
    try {
      const code = await blob.text();
      await runPython(code, (line, level) => { if (runId.current === id) push(line, level); });
      if (runId.current === id) { push('process finished', 'sys'); pulseVault(1); }
    } catch (e) {
      push(String((e as Error).message ?? e), 'error');
    } finally {
      setBooted(true);
      if (runId.current === id) setRunning(false);
    }
  };
  const stop = () => { runId.current++; setRunning(false); push('process detached (wasm runtime stays warm)', 'sys'); };
      ...
      {!running && (
        <div className="text-center py-8">
          <p className="font-mono text-[10px] tracking-[0.22em] uppercase text-slate-dim">
            python payload · cpython 3.12 on wasm {booted ? '· runtime warm' : '· first boot fetches the runtime'}
          </p>
          <button onClick={() => void start()} className="mt-5 font-mono text-[10px] tracking-[0.26em] uppercase border border-teal-ice/50 text-teal-ice px-6 py-2.5 hover:bg-teal-ice/10 transition-colors">
            run python
          </button>
...
function PdfRun({ file, blob }: { file: VaultFile; blob: Blob }) {
  ...
  return (
    <div>
      ...
      <iframe src={url} title={file.name} className="w-full h-[58vh] mt-2 bg-white border border-line/60" />
    </div>
  );
}
...
function ArchiveRun({ file, blob }: { file: VaultFile; blob: Blob }) {
  ...
  const importAll = async () => {
    ...
    try {
      ...
      const base = fsJoin(fsNorm(file.folder || '/'), rootName);
      ...
      for (const x of extracted) {
        ...
        const vf: VaultFile = {
          id, name, folder: fsJoin(base, dir), kind,
          mime: x.blob.type || 'application/octet-stream', size: x.blob.size,
          addedAt: Date.now(), realityId: state.activeRealityId,
        };
        if (kind === 'document' && vf.size < 1_500_000) vf.content = await x.blob.text();
        else { await putPayload(id, x.blob); vf.payloadRef = id; }
        files.push(vf);
      }
  ...
  if (appUrl) {
    return (
      <div>
        ...
        <iframe src={appUrl} title="archive app" sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-popups"
          className="w-full h-[58vh] mt-2 bg-white border border-line/60" />
      </div>
    );
  }
...
function RunView({ file }: { file: VaultFile }) {
  const [phase, setPhase] = useState<'probe' | 'ready' | 'legacy'>('probe');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      setBlob(b);
      setPhase(b && canExecute(file) ? 'ready' : 'legacy');
    })();
    return () => { alive = false; };
  }, [file.id, file.payloadRef, file.content]);
...
export default function VaultUI({ onClose }: { onClose: () => void }) {
  ...
  const importFiles = async (files: FileList | null, dest?: string) => {
    ...
    for (const f of Array.from(files)) {
      ...
      /* everything binary — media, archives, executables, games, datasets — keeps
         its real bytes in OPFS/IndexedDB so the execution engine can run it */
