...
- The **Universal Vault** (a black-hole-class object, the "Eventide Black
  ...
  compression accounting, and an in-vault terminal. It is also a **real
  execution engine**: it stores the true bytes of every file type and runs
  HTML apps/games (sandboxed iframe with sibling-asset bundling), JavaScript
  (isolated Web Worker), Python (CPython 3.12 on WASM via Pyodide), PDFs
  (native viewer) and ZIP archives (real listing/extraction/launch) — all
  client-side, and every vault action makes the black hole itself shudder
  and brighten.
...
src/
  ...
  db.ts                  re-export barrel for storage/indexedDB.ts (OPFS +
                         IndexedDB payload client)
  engine/
    engine.ts (~3560 ln) the universe: renderer/scene, body factory,
                         ...
                         frame tick, hover/pick, orbit lines, vault pulse
    cameraRig.ts         the advanced camera: damped orbit / inertial pan /
                         log-zoom rig (see §7.1)
    shaders.ts (~1420 ln) every GLSL program (see §7)
  physics/
  ...
  storage/
    ...
    indexedDB.ts         dual storage engine: OPFS first (raw binary payloads
                         as real disk files via navigator.storage.getDirectory()
                         writing 'payload-<id>.bin'), IndexedDB store
                         'eventide-universe'/'payloads' as fallback. Reads try
                         OPFS, fall back on miss.
    zip.ts (~110 ln)     dependency-free ZIP reader: scans the end-of-central-
                         directory record (22-byte sig 0x06054b50, up to 64KB
                         comment), walks central-directory headers 0x02014b50
                         into ZipEntry[] (path/dir/size/method/localOffset),
                         inflates entries via native DecompressionStream
                         ('deflate-raw') — method 8 — or slices stored entries
                         (method 0). unzipAll() has a 256MB inflation safety
                         budget.
    executors.ts (~240 ln) the Vault Execution Engine (see §10.2): runner
                         detection, blob resolution (OPFS payload → inline
                         content → data URL), HTML app bundler (sibling asset
                         rewrite + import maps), JS Web-Worker runner with
                         piped console, Pyodide Python runtime (v0.26.4 via
                         jsDelivr, cached after first fetch), archive
                         helpers, and pulseVault() event bridge to the 3D
                         engine ('eventide-vault-pulse' CustomEvent).
  ui/
    ...
    VaultUI.tsx (~4870 ln)    the vault OS: file grid + folders, identity
                              ...
                              and the RunView execution surface (§10.2)
...
- **orbit lines** (planet / dwarf / vault kinds): a `THREE.LineLoop`
  (`LineBasicMaterial` `#8ba1c4`, transparent, opacity 0) built in
  `buildBody` from exactly **one full revolution sampled in 256 equal time
  steps** — `periodDays = TAU / speed`, `simStep = (i/256) · periodDays`,
  each point through the same `calculateKeplerPosition` used for the live
  body, so the line is a smooth ellipse that always passes through the
  planet. Opacity is **hover-driven only**: `hoverT` eases toward 1 when the
  raycast hovers the body, target opacity `hoverT · sysW · 0.22`, hidden
  below 0.01 — a click can never pin it. (Critical fix: an earlier version
  sampled a fixed 10,000-day window in 160 steps, which alias-wrapped
  fast-orbiting bodies into star scribbles and hexagons — see §14.)
...
- **Kamui jutsu — the only bridge between the stages (Naruto-accurate
  ...
  scripted 3.6s flight (`kamuiFlight` 0→1) owns the camera completely, in
  four beats: **THE TEAR** — reality rips at a single point (the web's
  heart); **THE SUCK** — a consumption wave expands outward from that tear
  (`uVortexR` 45k → 525k in the web's materials), so the **nearest
  structures spiral into the swirl and are consumed first**, then the wave
  reaches farther ones — GPU vertex swirl (`pointsVert` + the web filament
  shader) bends everything around the tear in a black-hole whirlpool while
  the sky tears with it, and the focus drifts **along +z** into the vortex;
  **THE TUNNEL** — the dial dives, the fov blows wide open
  (`kamuiWarpFx` → +28°) and the screen-space swirl hits full strength: the
  teleport itself; at t = 0.55 the **stage swaps** under the full-screen
  vortex (the web is restored intact for the return trip) and the camera
  emerges from the second portal — **the marble's glass** — being **ejected
  along −z** (the focus sweeps out of the glass with a cubic ease + overshoot
  bump) to the arrival frame at the active reality's Pocket Cosmos Marble.
  Outward scrolling is swallowed mid-jutsu and wheel momentum is killed on
  landing. Exactly **one** jutsu per crossing, both directions (the return
  swaps web↔multiverse symmetrically via `warpDir` + `postWarpZoom`).
...
- **Cold-open intro**: the first 5.2 seconds are a Kamui arrival, not a text
  screen. Darkness with the round tunnel rotating around you (decelerating
  torque, walls parting — reusing the warp tunnel), then **your star
  ignites** (group scale pop + a corona flash via `uBoost`), the system
  builds one world at a time (bodies staggered by birth, `fadeTarget` wave),
  the sky arrives last, and the camera pulls back from the star to the home
  framing — all dials and fades on the existing scene (zero new GPU cost;
  shaders are precompiled at boot so it never stalls). `bootIntro` releases
  at 5.2s and everything hands back to the normal state machine.
- **Portal squeeze**: rendered distance scales `(1 − ease·0.45)` during the
  warp; FOV kicks 50 → 64 (engine-side), plus **+28° during the Kamui
  tunnel** (`kamuiWarpFx`).
...
- **Files**: **every file type keeps its real bytes.** On import
  (`importFiles` in `VaultUI.tsx`): small documents (< 1.5 MB) inline as
  `content`; everything else — media, archives, executables, games,
  datasets, PDFs — written to OPFS/IndexedDB under `payloadRef` (images also
  get a tiny inline `thumb`); `sealed` stubs only occur on storage failure.
...
### 10.1 Execution engine — architecture

`src/storage/executors.ts` turns stored payloads into live processes,
entirely client-side. Pipeline:

```ts
detectRunner(file): 'web-app' | 'javascript' | 'python' | 'pdf' | 'archive' | null
  pdf       ← ext/mime 'pdf'
  archive   ← zip/rar/7z/tar/gz or kind 'archive'
  web-app   ← html/htm
  javascript← js/mjs
  python    ← py

resolveBlob(file): Blob   // payloadRef → OPFS/IndexedDB; else inline
                           // content; else data: URL fetch
```

The Viewer (`VaultUI.tsx`) routes `archive | exe | application | game`
kinds **and any `*.pdf`** through `RunView`, which probes real bytes and
falls back to the legacy `SandboxLaunch` materialization sandbox for
pre-engine `sealed` stubs (labeled "legacy object"). Files with real bytes
but no runtime (e.g. Windows `.exe`) show an honest "no in-browser runtime"
card with download — browsers cannot execute native binaries.

### 10.2 Runners (exact behavior)

- **web-app** (`WebAppRun`): the HTML payload is bundled and run in a
  sandboxed iframe (`sandbox="allow-scripts allow-pointer-lock allow-forms
  allow-modals allow-popups"` — deliberately **no allow-same-origin**, so
  the guest can never touch vault storage). `bundleWebApp()` parses the
  document with DOMParser and rewrites every relative `src`/`href` that
  matches a sibling vault file from the *same folder* to a live blob URL
  (honoring `<base href>`; absolute/protocol refs untouched); if the app
  uses `<script type="module" src>`, it prepends an **import map** mapping
  sibling `.js` paths to blob URLs so ES-module imports resolve. Result is
  served as a `text/html` blob URL. "reboot" re-bundles (fresh nonce).
- **javascript** (`JsRun`): executed in a dedicated **Web Worker** built
  from a shim (console.log/info/debug/warn/error/table + onerror +
  onunhandledrejection all postMessage back) prepended to the payload.
  Console lines stream into a terminal pane (400-line ring, color-coded
  info/warn/error); terminate kills the worker. No DOM, no vault access.
- **python** (`PyRun`): real **CPython 3.12 on WASM via Pyodide v0.26.4**
  (script injected from jsDelivr on first run, ~10 MB, browser-cached
  afterwards; `ensurePython` memoizes the boot promise). stdout/stderr are
  piped through `setStdout/setStderr({batched})` into the same terminal
  pane; "terminate" detaches (the WASM runtime stays warm for the next run).
- **pdf** (`PdfRun`): the stored bytes as an object URL in a plain iframe —
  the browser's native PDF viewer.
- **archive** (`ArchiveRun`): the **real** zip central directory is listed
  (path + uncompressed size). Per-entry "get" inflates just that entry
  (stored entries sliced, deflate via `DecompressionStream('deflate-raw')`)
  and downloads it. "extract all → vault" inflates every entry (256 MB
  budget guard) and imports each as a real VaultFile under
  `/<archivename>/<nested/path>`, choosing inline content for small text
  files and OPFS payloads otherwise. If the archive contains any `.html`,
  a **"run app inside"** button extracts everything in memory, picks the
  entry point (shallowest `index.html`, else shallowest html) and launches
  it through the same bundler + sandboxed iframe — zipped HTML5 games run
  as-is, with all sibling assets (js/css/img) resolvable.

### 10.3 The black hole feeds (engine bridge)

Every vault event calls `pulseVault(intensity)` (import 0.6, archive-extract
0.8, run 1.0), which dispatches a `CustomEvent('eventide-vault-pulse')` on
`window`. The 3D engine listens from construction and adds intensity to
`vaultPulse` (clamped 1.5). In the frame tick, the Eventide vault body's
lattice rings (`r1`, `r2` — the two tilted tori) shudder
(`sin(clockT·12)·0.015·pulse` scale) and their emissiveIntensity flares to
`1.8 + pulse·2.2`, decaying at `0.5/s` back to baseline 1.8. Storage and
execution are physically visible in the universe.
...
Reconstruction must reproduce the project **as it is now**, including these
deliberate changes:
...
- **Orbit lines sample one full revolution, not a fixed time window.**
  The original `buildBody` orbit line sampled `simStep = (i/160) · 10000`
  days. Because planets have different `orbit.speed`, fast bodies wrapped
  multiple laps between samples (samples landed at chaotic anomalies → a
  star-shaped scribble of chords), Earth-speed bodies got ~6 samples per
  lap (a hexagon), and slow bodies rendered chunky many-sided polygons —
  every planet showed a *different* wrong shape on hover. The fix samples
  exactly one orbital period in 256 steps (§7), so every orbit renders as
  a smooth Kepler ellipse at any speed. **Do not reintroduce fixed-window
  sampling.**
- **The fake archive/executable sandbox is replaced by real execution.**
  `ArchiveList` (randomly generated zip entries) was deleted; the scripted
  `SandboxLaunch` boot theater remains only as a labeled fallback for
  legacy pre-engine `sealed` stubs. Regenerators must implement the real
  runners (§10.1–10.3), not the theater.
...
7. **Execute, don't simulate — where the browser honestly can.** The vault
   stores true bytes for every file type and runs what runs natively in a
   browser (HTML apps, JS, Python/WASM, PDF, ZIPs); what cannot run (native
   .exe) is stored, offered for download, and *labeled* as out of runtime —
   never faked with theater.
